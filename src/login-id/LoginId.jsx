import React, { useState, useEffect } from 'react';
import LoginIdSDK from '@auth0/auth0-acul-js/login-id';
import { Card, CardHeader, CardContent } from '@components/Card';
import { Input } from '@components/Input';
import { Button } from '@components/Button';
import { ErrorMessage } from '@components/ErrorMessage';

/**
 * Login ID Screen
 * First step of login flow - captures username/email
 */
export const LoginIdScreen = () => {
  const [loginIdManager] = useState(() => {
    try {
      return new LoginIdSDK();
    } catch (error) {
      // Mock SDK for local development
      console.warn('Running in development mode with mocked Auth0 SDK');
      return {
        user: {},
        tenant: { name: 'Demo Tenant' },
        branding: {},
        transaction: {},
        getErrors: () => [],
        validateUsername: () => ({ isValid: true }),
        login: () => Promise.resolve(),
        submit: () => Promise.resolve()
      };
    }
  });
  const [username, setUsername] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const [errors, setErrors] = useState([]);
  const [contextData, setContextData] = useState(null);

  useEffect(() => {
    // Get context data from SDK
    try {
      const context = {
        user: loginIdManager.user,
        tenant: loginIdManager.tenant,
        branding: loginIdManager.branding,
        transaction: loginIdManager.transaction
      };
      setContextData(context);
      
      // Get initial errors if any
      const initialErrors = loginIdManager.getErrors() || [];
      setErrors(initialErrors);
    } catch (error) {
      console.error('Error initializing LoginIdScreen:', error);
    }
  }, [loginIdManager]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setErrors([]);

    try {
      // Validate username
      if (!username.trim()) {
        setErrors([{ message: 'Username or email is required' }]);
        setIsLoading(false);
        return;
      }

      // Call SDK login method
      await loginIdManager.login({ username: username.trim() });
      
      // Check for errors after submission
      const submissionErrors = loginIdManager.getErrors() || [];
      if (submissionErrors.length > 0) {
        setErrors(submissionErrors);
      } else {
        // Success - in production, Auth0 will handle the redirect
        console.log('Login ID submitted successfully');
      }
    } catch (error) {
      console.error('Login error:', error);
      setErrors([{ message: error.message || 'Failed to submit login' }]);
    } finally {
      setIsLoading(false);
    }
  };

  const handleSocialLogin = async (connection) => {
    setIsLoading(true);
    setErrors([]);

    try {
      await loginIdManager.continueWithFederatedLogin({ connection });
      console.log(`Social login with ${connection} initiated`);
    } catch (error) {
      console.error('Social login error:', error);
      setErrors([{ message: error.message || 'Social login failed' }]);
    } finally {
      setIsLoading(false);
    }
  };

  // Get alternate connections (social providers)
  const alternateConnections = contextData?.transaction?.alternateConnections || [];

  return (
    <Card>
      <CardHeader
        logo="A0"
        title="Welcome Back"
        description="Sign in to your account to continue"
      />

      <CardContent>
        {errors.length > 0 && <ErrorMessage errors={errors} />}

        <form onSubmit={handleSubmit} className="form">
          <Input
            label="Email or Username"
            type="text"
            name="username"
            id="username"
            value={username}
            onChange={(e) => setUsername(e.target.value)}
            placeholder="Enter your email or username"
            autoComplete="username"
            autoFocus
            required
            disabled={isLoading}
          />

          <Button 
            type="submit" 
            variant="primary" 
            loading={isLoading}
            disabled={isLoading || !username.trim()}
          >
            Continue
          </Button>
        </form>

        {alternateConnections.length > 0 && (
          <>
            <div className="divider">Or continue with</div>
            
            <div className="social-buttons">
              {alternateConnections.map((connection) => (
                <Button
                  key={connection.name}
                  variant="social"
                  onClick={() => handleSocialLogin(connection.name)}
                  disabled={isLoading}
                >
                  {getSocialIcon(connection.name)}
                  Continue with {connection.display_name || connection.name}
                </Button>
              ))}
            </div>
          </>
        )}

        <div className="text-center" style={{ marginTop: 'var(--space-lg)' }}>
          <span className="text-sm">Don't have an account? </span>
          <a href="#" className="link">Sign up</a>
        </div>
      </CardContent>
    </Card>
  );
};

// Helper function to get social provider icons
function getSocialIcon(provider) {
  const icons = {
    google: (
      <svg className="social-icon" viewBox="0 0 24 24" fill="currentColor">
        <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
        <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
        <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
        <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
      </svg>
    ),
    github: (
      <svg className="social-icon" viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>
    ),
  };
  
  return icons[provider.toLowerCase()] || icons.google;
}

export default LoginIdScreen;

// Auto-render in Auth0 ACUL environment
if (typeof window !== 'undefined') {
  const initScreen = () => {
    const container = document.getElementById('custom-prompt-container');
    if (container && !container.hasChildNodes()) {
      import('react-dom/client').then(({ createRoot }) => {
        createRoot(container).render(<LoginIdScreen />);
      });
    }
  };
  
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initScreen);
  } else {
    initScreen();
  }
}